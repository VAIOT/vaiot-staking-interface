...
import { TokenAmount, JSBI, ChainId } from '@pancakeswap/sdk'
...
// check if user is in blob and has not yet claimed VAI
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
  ...
  return new TokenAmount(vai, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} VAI`,
